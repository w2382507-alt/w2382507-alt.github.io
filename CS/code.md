...
- 变量
	- 持续性、作用域、链接性
		- 作用域
			...
			- 类作用域：class/struct定义内
			- 调用规则：
				1. 现在当前作用域内查找
				2. 找不到就向外层作用域查找
				3. 在作用域外调用需要作用域解析运算符 
		- 自动存储持续性：自动变量
			> 持续性：使用栈储存，在程序开始执行其所属的函数或代码块时被创建,在执
			> 行完函数或代码块时,它们使用的内存被释放
			> 作用域：局部
			> 链接性：无
			> 创建：在函数中声明
		- 静态存储持续性
			> 持续性：在程序整个运行过程中都存在，零初始化
			- 静态无链接性变量
				> 作用域：局部
				> 链接性：无
				> 创建：在代码块中，使用关键字static
				> 只会初始化一次，函数多次调用时他的值也不会改变
			- 静态内部链接性变量：静态全局变量
				> 作用域：全局
				> 链接性：内部
				> 创建：在代码块外，使用关键字static或关键字const
				> 静态内部链接性变量会隐藏静态外部链接性变量
			- 静态外部链接性变量：外部全局变量
				> 作用域：全局
				> 链接性：外部。在外部文件使用时用extern关键字，引用的作用域决定于引用的位置。exturn关键字可以放在头文件中，只是声明不是定义
				> 创建：在代码块外
		- 线程存储持续性
		- 动态存储持续性
			- 持续性：用new运算符分配的内存将一直存在,直到使用delete运算符将其释放或程序结束为止。这种内存的存储持续性为动态,有时被称为自由存储(frec store)或堆(heap)。
			- new运算符( only c++)
				- 创建指针
				    - 动态变量
	...
	- string类（only c++）
		...
		- 赋值、拼接、附加
		    ...
		    - str1 += str2
	    - string 类一定要加std::
	    - string类不依赖\0判断结尾，只有在以c风格字符串赋值时会包含\0，所有不要用\0来判断结尾，正确的遍历方法是 `i < str.size()`
...
- 复合类型
	- 数组array
	    - 声明与初始化
		    ...
		    - 不完全初始化剩余元素默认为零，不写=｛｝会导致数组没有初始化，不会自动设置为0，是随机值！
    - c-风格字符串
      - 以 '\0' 结尾的char数组
    ...
    - 数组替代品（only c++）
	    - vector
	        - @vector std::
	        - std::vector\<typeName\> vec (n\_elem);
		        - n\_elem可以是变量，初始大小，创建后元素的值为默认0
		    - 类成员方法
			    - push_back()：向尾部添加元素
			    - size()：返回当前元素数量
	...
	- 枚举（only c++）enum
	    ...
	    - 只能将定义枚举时用的枚举量赋给这种枚举的变量
		    - 赋值时不能使用整数类型赋值，只能用枚举类型
	- 作用域内枚举（c++11）enum class
		- 该类型枚举的作用域只在类内，在外部使用需要使用作用域解析符`::`
- 指针
	- 初始化
	    - int \* pt = new int;
	    - int \* pt = &valueName;
...
- 编译
	- 程序组成结构
		- 头文件
			...
			- 使用`#define`或`const` 定义的符号常量
				> const修饰变量在链接后创建的是内部链接性的变量，相当于每个文件有独立的一个const变量，不违反单定义规则
			- 类声明
			- 结构声明
			- 模板声明
			- 内联函数(inline)
	...
	- 编译过程
		- 预处理：生成.i文件
			- 展开`#include`头文件
				- `#include "文件名"` 将会首先查找当前工作目录
				- `#include <文件名>` 将在存储标准头文件的主机系统文件系统中查找
			- 处理`#define`宏替换
			- 删除注释
...
- 类
	...
	- 成员函数
		...
		- const 成员函数
			...
			- 不会修改类对象当中成员的函数，相当于this指针是const类型的
